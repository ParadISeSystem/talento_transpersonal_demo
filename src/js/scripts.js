...
function initSliders() {
    // Initialize course slider with existing HTML structure
    initCourseSlider();
}
...
function initCourseSlider() {
    const sliderTrack = document.querySelector('#cursos .slider-track');
    const prevBtn = document.querySelector('#cursos .slider-prev');
    const nextBtn = document.querySelector('#cursos .slider-next');
    const dots = document.querySelectorAll('#cursos .dot');
    const slides = document.querySelectorAll('#cursos .curso-slide');
    ...
    function getVisibleSlides() {
        ...
        if (width >= 768) return 2;  // md: show 2 slides
        return 1; // sm: show 1 slide
    }
    ...
    function updateSlider() {
        ...
        // Update dots
         dots.forEach((dot, index) => {
             ...
             if (visibleSlides === 1) {
                 // Mobile: each dot represents one slide
                 isActive = index === currentSlide;
             } else if (visibleSlides === 2) {
                 // Tablet: each dot represents a group
                 const groupIndex = Math.floor(currentSlide / 2);
                 isActive = index === groupIndex;
             } else {
                 // Desktop: show all slides, highlight current position
                 isActive = index === currentSlide;
             }
             
             dot.classList.toggle('bg-accent-blue', isActive);
    ...
    // Dot navigation
     dots.forEach((dot, index) => {
         dot.addEventListener('click', () => {
             ...
             // Calculate the target slide based on visible slides
             let targetSlide = index;
             if (visibleSlides === 1) {
                 // Mobile: each dot represents one slide
                 targetSlide = Math.min(index, maxSlide);
             } else if (visibleSlides === 2) {
                 // Tablet: each dot represents a group of slides
                 targetSlide = Math.min(index * 2, maxSlide);
             } else {
                 // Desktop: show all slides, dots for navigation
                 targetSlide = Math.min(index, maxSlide);
             }
...
/**
 * Legacy Slider Class (kept for compatibility)
 */
class Slider {
